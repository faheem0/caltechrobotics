...
'' PS2 Controller Driver v1.2b  
' does not freeze anymore, but doesn't work
...
loop          mov attPin, #0 wz,nr   'set attn LOW to select controller             
              ...
              mov psxOut, #startByte     'send 'start' command
              call #psxTxRx
...
' returns: psxIn                           
psxTxRx       mov psxIn, #0         'clear byte
...
txrxloop      test psxOut, #1 wc    'place bit from psxOut on cmdPin
              ...
              waitcnt time, #0
              mov clkPin, #0 wz,nr  'use Z Flag for clk                       
